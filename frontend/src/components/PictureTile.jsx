...
const PictureTile = ({ item , fade = false , selectMode = false , selectedList }) => {
	...
	const imgElement =(
		...
		>
			<img
				className={`object-cover object-center thumb-img rounded-xl`}
				style={{
		...
		<div className="relative group border border-purple-700/45 rounded-xl transition-all ease-in-out duration-150">
			{
				...
				<section className="relative object-contain ">
					<div
						className="absolute text-4xl text-center top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%] text-amber-300 opacity-0 group-hover:opacity-85 z-20 transition-all duration-[0.35s] mt-[15%] group-hover:mt-0 capitalize font-black mix-blend-lighten "
					>
						{item?.title}
					</div>
					...
					<div className="absolute bg-purple-500 opacity-0 top-0 left-0 h-full w-full group-hover:opacity-20 mix-blend-color-dodge transition-all duration-300 z-10 rounded-xl" />

					<div className="absolute bg-purple-500 opacity-0 top-0 left-0 h-full w-full group-hover:opacity-30 mix-blend-luminosity transition-all duration-300 z-10 rounded-xl" />
