	...
	return(
		...
		>
			<div className="mt-1 flex flex-col gap-2" >
				<img src={pfp} alt="pfp" className="rounded-full w-14 aspect-square object-cover" />
...

				{
					routes.map((item, index) => (
						<Link
							...
							className={`${item.isActive ? " bg-purple-500/75 text-white" : " hover:bg-purple-500/40 hover:text-white"}  p-3 rounded-lg transition-all ease-in duration-150`}
						>
							{item.icon}
						</Link>
					))
